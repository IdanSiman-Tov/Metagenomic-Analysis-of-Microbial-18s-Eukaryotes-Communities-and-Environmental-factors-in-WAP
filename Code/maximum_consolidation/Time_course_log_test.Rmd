...
```{r}
data_for_pca["Temperature...K."]<-data_for_pca$Temperature...C. + 273.16
data_for_pca<-data_for_pca[,-1]

data_for_pca$`Fluorescence..mg.m³.`<-data_for_pca$`Fluorescence..mg.m³.` + 0.0904

data_for_pca$`Phaeopigment..mg.m³.`<-data_for_pca$`Phaeopigment..mg.m³.` + 1.22675401
...
Group_1_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Archaeplastida.Chlorophyta.and.otherArchaeplastida|Stramenopiles.Dictyochophyceae.all"))

# Filtering to only include the "Group 2" from percentages
Group_2_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Excavata.Discoba.Jakobida|Cryptophyceae.Geminigera.and.otherCryptophyceae|Stramenopiles.Diatomea.all|Eukaryota_other"))
...
Group_4_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Rhizaria.Cercozoa"))
```
...
```{r}
Enviromental_data_melt[,3]<- log10(Enviromental_data_melt[,3])
```
...
Group_1_time_heatmap<-ggplot() +
  ...
  # Stramenopiles.Dictyochophyceae.all with temp
  geom_raster(data = filter(Enviromental_data_melt, Enviromental == "Temperature...K."), aes(x = Date, y = Enviromental, fill = Value)) +
  scale_fill_gradient2(low = "white", mid = "orangered2", high = "darkslategrey", midpoint = 2.4375, name = "Temperature...K.") +
  new_scale_fill()+

  
  # Stramenopiles.Dictyochophyceae.all with Density..kg.m³
  geom_raster(data = filter(Enviromental_data_melt, Enviromental == "Density..kg.m³."), aes(x = Date, y = Enviromental, fill = Value)) +
  scale_fill_gradient2(low = "white", mid = "deepskyblue3", high = "darkslategrey", midpoint = 3.01145, name = "Density..kg.m³.") +
  new_scale_fill()+
  ...
  scale_fill_gradient2(low = "white", mid = "slateblue", high = "darkslategrey", midpoint = 0.15, name = "Phosphate..µmol.L.") +
  new_scale_fill() #+
  # theme(plot.margin.bottom = 100)
...
```{r}
ggarrange(Group_1_time_course, Group_1_time_heatmap, heights = c(2, 0.5), ncol = 1, nrow = 2, align = "hv", hjust = 10)+
  theme(plot.margin = margin(0,0,100,0, "pt")) 
```
