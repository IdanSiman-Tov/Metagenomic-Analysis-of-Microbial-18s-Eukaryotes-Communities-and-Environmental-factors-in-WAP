...
Group_1_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Archaeplastida.Chlorophyta.and.otherArchaeplastida|Rhizaria.Cercozoa|Stramenopiles.Diatomea.ME-Euk-FW10|Stramenopiles.Dictyochophyceae.Pedinellales"))

# Filtering to only include the "Group 2" from percentages
Group_2_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Excavata.Discoba.Jakobida|Haptophyta.non_Phaeocystis|Eukaryota_other"))

# Filtering to only include the "Group 3" from percentages
Group_3_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Cryptophyceae.Cryptomonadales.Geminigera|Picozoa.Picomonadida|Stramenopiles.otherStramenopiles"))

# Filtering to only include the "Group 4" from percentages
Group_4_metagenome_taxa_percentage_time_course_log <- metagenome_taxa_percentage_time_course_log %>% filter(str_detect(Taxa, "Cryptophyceae.otherCryptophyceae|Stramenopiles.Dictyochophyceae.Dictyochales.and.Florenciellales.and.otherDictyochophyceae|Stramenopiles.MAST-2.and.MAST-3"))
```
...
Group_1_time_heatmap<-ggplot() +
  ...
  # Rhizaria.Cercozoa and primary prod
  ...
  # Archaeplastida.Chlorophyta.and.otherArchaeplastida and phospate
